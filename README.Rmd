...
`remove_na_cols()` deletes a column that consists only of `NA`. `remove_na_rows()` deletes a rows that consists only of `NA`. `consolidate_cols()` retrieves a vector of values where each row of the specified data frame consists of NA and one other value, either with NA removed only or with `NA` if all values in the row are `NA`. `check_onevalue_cols()` is check funcion within `consolidate_cols()`.
